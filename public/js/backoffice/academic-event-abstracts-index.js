document.addEventListener('DOMContentLoaded', () => {
    const root = document.getElementById('bo-academic-event-abstracts-index');
    const selectAll = document.getElementById('select-all');
    const rowChecks = document.querySelectorAll('.bo-ae-abs-checkbox');
    const bulkBtn = document.getElementById('bulk-delete-btn');
    const deleteForms = document.querySelectorAll('#bo-academic-event-abstracts-index .js-delete-confirm-form');
    const perPageSelect = document.querySelector('.bo-ae-abs-per-page');
    const perPageForm = document.getElementById('bo-ae-abs-per-page-form');
    ...
    deleteForms.forEach((deleteForm) => {
        deleteForm.addEventListener('submit', (event) => {
            const ok = window.confirm('정말 이 초록을 삭제하시겠습니까?');
            if (!ok) {
    ...
    bulkBtn?.addEventListener('click', async () => {
        ...
        selectedIds.forEach((id) => formData.append('abstract_ids[]', id));
