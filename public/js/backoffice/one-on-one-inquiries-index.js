/**
 * 1:1 문의 목록: 전체 선택, 선택 삭제, 단건 삭제 확인, 페이지 크기 변경
 */
document.addEventListener('DOMContentLoaded', () => {
    const root = document.getElementById('bo-one-on-one-inquiries-index');
    const selectAll = document.getElementById('select-all');
    const rowChecks = document.querySelectorAll('.bo-one-on-one-inquiry-checkbox');
    ...
    deleteForms.forEach((form) => {
        form.addEventListener('submit', (event) => {
            const ok = window.confirm('정말 이 문의를 삭제하시겠습니까?');
            if (!ok) {
