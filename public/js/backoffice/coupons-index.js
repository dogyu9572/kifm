/**
 * 쿠폰 목록: 전체 선택, 선택 삭제, 단건 삭제 확인, 페이지 크기 변경
 */
document.addEventListener('DOMContentLoaded', () => {
    const root = document.getElementById('bo-coupons-index');
    const selectAll = document.getElementById('select-all');
    const rowChecks = document.querySelectorAll('.bo-coupon-checkbox');
    const bulkBtn = document.getElementById('bulk-delete-btn');
    const deleteForms = document.querySelectorAll('#bo-coupons-index .js-delete-confirm-form');
    const perPageSelect = document.querySelector('.bo-coupon-per-page');
    const perPageForm = document.getElementById('coupon-per-page-form');
    ...
    if (bulkBtn && rowChecks.length > 0) {
        bulkBtn.addEventListener('click', async () => {
            ...
            ids.forEach((id) => formData.append('coupon_ids[]', id));
