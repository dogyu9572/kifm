(function () {
    const form = document.getElementById('bo-academic-event-registration-form');
    if (!form) {
        return;
    }

    const cardGuideEl = document.getElementById('bo-ae-reg-card-guide');
    const bankBlockEl = document.getElementById('bo-ae-reg-bank-block');
    const receiptDetailEl = document.getElementById('bo-ae-reg-receipt-detail');
    ...
    const syncPaymentMethodPanels = () => {
        ...
        const hideCardGuide = showBank || method === 'onsite';
        if (cardGuideEl) {
            cardGuideEl.classList.toggle('bo-hidden', hideCardGuide);
        }
        if (bankBlockEl) {
    ...
    memberSelectorRoot?.addEventListener('bo-member-selected', (event) => {
        ...
        if (nameInput) {
            nameInput.value = detail.name || detail.label || '';
        }
        if (licenseInput) {
