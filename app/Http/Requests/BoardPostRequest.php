...
class BoardPostRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if (is_string($content) && $this->isBlankHtmlContent($content)) {
            $this->merge(['content' => '']);
        }
    }

    private function isBlankHtmlContent(string $content): bool
    {
        if (preg_match('/<(img|iframe|video|audio|table)\b/i', $content)) {
            return false;
        }
