...
use App\Services\Backoffice\MemberCsvImporter;
use Illuminate\Console\Command;

class ImportMembersFromCsvCommand extends Command
{
    protected $signature = 'members:import-csv
        {path? : CSV 경로 (기본: docs/data-migration/회원관리.csv)}
        {--dry-run : DB 반영 없이 건수만 집계}
        {--limit= : 최대 처리 행 수}
        {--on-duplicate=skip : skip 또는 update}';

    protected $description = '회원관리.csv(CP949)를 users 테이블로 이관합니다.';

    public function handle(MemberCsvImporter $importer): int
    {
        $path = $this->argument('path')
            ?: base_path('docs/data-migration/회원관리.csv');
        ...
        $this->table(['항목', '건수'], [
            ['신규(또는 dry-run 예상)', $stats['created']],
            ['갱신', $stats['updated']],
            ['스킵', $stats['skipped']],
        ]);
