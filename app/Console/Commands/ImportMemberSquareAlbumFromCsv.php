...
class ImportMemberSquareAlbumFromCsv extends Command
{
    /**
     * @var string
     */
    protected $signature = 'posts:import-member-square-album
        {path : 학회앨범 CSV 경로 (절대 경로 또는 프로젝트 기준 상대 경로)}
        {--encoding=UTF-8 : 원본 CSV 인코딩}
     ...
     * @var string
     */
    protected $description = '레거시 커뮤니티 학회앨범 CSV를 board_member_square_album 테이블로 이관합니다.';

    private const TABLE = 'board_member_square_album';

    private const ATTACHMENT_TARGET_DIR = 'legacy/attachments/member_square_album';
    ...
    public function handle(): int
    {
        ...
        $this->info(sprintf(
            '완료. 생성=%d / 갱신=%d / 스킵=%d / 본문이미지 복사=%d, 누락=%d / 첨부 복사=%d, 누락=%d / 썸네일 자동지정=%d%s',
            $stats['created'],
            $stats['updated'],
            $stats['skipped'],
            $stats['image_copied'],
     ...
     * 외부 URL이거나 storage 경로가 아닌 경우 null.
     */
