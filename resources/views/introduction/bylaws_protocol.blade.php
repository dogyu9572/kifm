...
<section class="scon" aria-labelledby="bylaws-heading">
	<div class="inner">
		...
		<div class="bylaws_top">
			<form method="GET" action="{{ route('introduction.bylaws_protocol') }}" data-committee-bylaws-form>
				<label for="committee-protocol-select" class="sound_only">위원회 선택</label>
				<select name="committee_id" id="committee-protocol-select" class="text" data-committee-bylaws-select>
		...
		<div class="bylaws_wrap">
			@if ($selectedCommittee)
				@if (trim((string) $selectedCommittee->protocol) !== '')
					{!! $selectedCommittee->protocol !!}
				@else
					<article class="bylaws_item">
						<div class="con">
							<p>등록된 업무 프로토콜이 없습니다.</p>
						</div>
					</article>
				@endif
