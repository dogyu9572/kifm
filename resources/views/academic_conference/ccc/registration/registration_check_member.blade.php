	...
	<div class="inner">
		...
		<div class="member_inbox">
			<ul class="tabs type_member">
				<li class="on"><a href="{{ $conferenceBaseUrl }}/registration/check_member">회원 등록</a></li>
				<li><a href="{{ $conferenceBaseUrl }}/registration/check_non_member">비회원 등록</a></li>
			</ul>
			
			<p class="tb tac">회원으로 신청하신 경우, 로그인 후 확인하실 수 있습니다.</p>
			<form action="{{ route('member.login.store') }}" method="POST" class="inputs gbox">
				@csrf
				<input type="hidden" name="intended" value="{{ $conferenceBaseUrl }}/registration/result">
				...
				@if (auth()->check() && auth()->user()?->role === 'user')
					<a href="{{ $conferenceBaseUrl }}/registration/result" class="btn btn_wkk">등록 내역 확인</a>
				@else
