...
@section('content')
@php
	$member = $currentMember ?? auth()->user();
	...
	<div class="inner">
		...
		<div class="member_inbox">
			...
			<div class="inbox input_wrap">
				...
				@enderror
				<form action="{{ route('academic_conference.site.abstract.store_member', $event->folder_name) }}" method="post" enctype="multipart/form-data" data-abstract-form>
					@csrf
					<fieldset>
						...
						<ul class="inputs float">
							...
							<li>
								...
								<input type="tel" id="user_phone" name="author_phone" class="text" value="{{ $member?->workplace_phone }}" readonly>
							</li>
							<li>
								<label for="user_tel" class="tit">휴대폰번호</label>
								<input type="tel" id="user_tel" name="author_mobile" class="text" value="{{ old('author_mobile', $member?->phone_number) }}" readonly>
								@error('author_mobile')
									<p class="c_red" role="alert">{{ $message }}</p>
								@enderror
							</li>
							<li>
								...
								<input type="email" id="user_email" name="author_email" class="text" value="{{ old('author_email', $member?->email) }}" readonly>
								@error('author_email')
									<p class="c_red" role="alert">{{ $message }}</p>
								@enderror
							</li>
