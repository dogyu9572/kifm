...
@section('content')
@php
	$submitPeriod = $event->abstract_start && $event->abstract_end
		? $event->abstract_start->format('Y년 m월 d일') . ' ~ ' . $event->abstract_end->format('Y년 m월 d일')
		: '관리자 설정 후 안내됩니다.';
	$revisionEnd = $event->abstract_revision_end ? $event->abstract_revision_end->format('Y년 m월 d일') : null;
@endphp
<main class="sub_area">
	...
	<div class="inner">
		...
		<section class="info_section" aria-labelledby="pre-submit-title">
			<div class="stit">
				...
				@if ($abstractBookUrl)
					<a href="{{ $abstractBookUrl }}" class="btn_abso btn_download" target="_blank" rel="noopener">초록 양식 다운로드</a>
				@endif
			</div>
			<div class="gbox">
				@if ($event->submission_guide)
					{!! $event->submission_guide !!}
				@else
					<ul class="dots_list">
						<li>제출 마감: {{ $submitPeriod }}</li>
		...
		<section class="info_section" aria-labelledby="pre-registration-title">
			...
			<div class="gbox">
				@if ($event->abstract_notes)
					{!! $event->abstract_notes !!}
				@else
		...
		<div class="btns_btm">
			<a href="{{ $conferenceBaseUrl }}/abstract/submission" class="btn btn_wbb">초록 신청</a>
			<a href="{{ $conferenceBaseUrl }}/abstract/check_member" class="btn btn_kwg">초록신청 조회</a>
		</div>
