	...
	<div class="inner">
		...
		<div class="member_inbox">
			<ul class="tabs type_member">
				<li><a href="{{ $conferenceBaseUrl }}/abstract/check_member">회원 등록</a></li>
				<li class="on"><a href="{{ $conferenceBaseUrl }}/abstract/check_non_member">비회원 등록</a></li>
			</ul>
			
			<p class="tb tac">초록 접수 시 입력하신 정보로 조회하실 수 있습니다.</p>
			<form action="{{ route('academic_conference.site.abstract.check_non_member', $event->folder_name) }}" method="POST" class="inputs gbox">
				@csrf
				<h2 class="sound_only">초록 접수 조회 정보 입력</h2>
				@error('lookup')
				...
				@enderror
                <div class="input_row">
                    <label for="abstract-password">초록 접수 비밀번호</label>
                    <input type="password" id="abstract-password" name="lookup_password" class="text w100p" placeholder="초록 접수 비밀번호를 입력해주세요." required>
                </div>
				@error('lookup_password')
					<p class="c_red" role="alert">{{ $message }}</p>
				@enderror
				<button type="submit" class="btn btn_wkk mt">초록접수 조회</button>
			</form>
