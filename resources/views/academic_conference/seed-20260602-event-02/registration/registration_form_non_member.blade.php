			...
			<form action="{{ route('academic_conference.site.registration.store_non_member', $event->folder_name) }}" method="post" id="academic-registration-form" data-coupon-url="{{ route('academic_conference.site.registration.coupon', $event->folder_name) }}">
                    ...
                    <ul class="inputs">
                        ...
                        <li>
                            ...
                            @error('address_postcode')
                                <p class="c_red" role="alert">{{ $message }}</p>
                            @enderror
							<input type="text" id="address_base" name="address_base" class="text" value="{{ old('address_base') }}" placeholder="주소를 입력해주세요" required title="주소 입력 필수" readonly>
                            @error('address_base')
                                <p class="c_red" role="alert">{{ $message }}</p>
                            @enderror
							<input type="text" id="address_detail" name="address_detail" class="text" value="{{ old('address_detail') }}" placeholder="상세주소를 입력해주세요" required title="상세주소 입력 필수">
                ...
                <fieldset>
					...
					<ul class="radios float">
                        ...
                        @empty
                            <li>
                                <p class="c_red">등록 가능한 비회원 결제 항목이 없습니다. 관리자에게 문의해주세요.</p>
                            </li>
                        @endforelse
					</ul>
