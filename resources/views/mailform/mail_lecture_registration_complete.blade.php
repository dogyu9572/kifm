...
@php
	...
	$memberName = $paymentModel?->name ?: ($member?->name ?: '-');
	$lectureName = $paymentModel?->training?->title ?: '-';
	$planName = $paymentModel?->items?->pluck('item_name')->filter()->implode(', ') ?: '-';
	$requestedAt = $paymentModel?->registered_at?->format('Y년 n월 j일 H:i:s') ?: '-';
	$depositExpectedDateText = $paymentModel?->bank_deposit_date?->format('Y년 n월 j일') ?: '-';
	$amountText = $paymentModel ? number_format((int) $paymentModel->total_amount).'원' : '-';
	...
	$depositorName = $paymentModel?->bank_depositor ?: $memberName;
@endphp
	...
	<tbody>
		...
		<tr>
			<td style="padding:0 40px 48px;">
				<div style="font-size:24px; color:#222; font-weight:700; line-height:1.4; letter-spacing:-.02em;">연수강좌 신청 완료 안내</div>
				<p style="font-size:16px; color:#222; line-height:1.5; letter-spacing:-.02em; margin:16px 0 40px;">
					안녕하세요, 대한기능의학회입니다.<br>
					회원님께서 신청하신 연수강좌의 무통장 입금 신청이 접수되었습니다.<br>
					입금 확인 후 신청이 최종 승인됩니다.<br>
					아래에서 신청 상세 내용을 확인해 주시기 바랍니다.
				</p>
				<div style="background:#f8f8f8; border-radius:8px; padding:20px 24px;">
					<table style="table-layout:fixed; width:100%; border-collapse:collapse; border-spacing:0;">
						<tbody>
							<tr>
								<th style="font-size:14px; color:#222; font-weight:400; line-height:1.5; letter-spacing:-.02em; padding:4px 0; width:115px; text-align:left;">연수교육명</th>
				...
				<div style="text-align:center; padding:54px 0 14px;">
					<a href="{{ url('/mypage/participation_history') }}" target="_blank" style="font-size:14px; color:#fff; font-weight:600; line-height:1; padding:14px 45px; background:#0088B8; border-radius:8px; text-decoration:none;">신청 내역 확인하기</a>
				</div>
			</td>
		</tr>
		<tr>
