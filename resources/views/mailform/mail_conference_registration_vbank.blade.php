...
@php
    ...
    $member = $registrationModel?->member;
    $memberName = $registrationModel?->name ?: ($member?->name ?: '-');
    $conferenceName = $event?->title ?: '-';
    $registrationNo = $registrationModel?->registration_no ?: '-';
    $eventStartAt = $event?->start_at?->format('Y년 n월 j일 H:i') ?: '-';
    $eventEndAt = $event?->end_at?->format('H:i') ?: '-';
    $eventDateText = ($eventStartAt !== '-') ? $eventStartAt.' - '.$eventEndAt : '-';
    $eventPlace = $event?->venue ?: '-';
    $planName = $registrationModel?->items?->pluck('item_name')->filter()->implode(', ') ?: '-';
    $requestedAt = $registrationModel?->registered_at?->format('Y년 n월 j일 H:i:s') ?: '-';
    $depositExpectedDateText = $registrationModel?->bank_deposit_date?->format('Y년 n월 j일') ?: '-';
    $amountText = $registrationModel ? number_format((int) $registrationModel->total_amount).'원' : '-';
    ...
    $depositorName = $registrationModel?->bank_depositor ?: $memberName;
@endphp
        ...
        <tr>
            <td style="padding:0 40px 48px;">
                ...
                <p style="font-size:16px; color:#222; line-height:1.5; letter-spacing:-.02em; margin:16px 0 40px;">
                    안녕하세요, 대한기능의학회입니다.<br>
                    {{ $conferenceName }} 사전등록 무통장 입금 신청이 접수되었습니다.<br>
                    입금 확인 후 등록이 최종 승인됩니다.<br>
                ...
                <div style="background:#f8f8f8; border-radius:8px; padding:20px 24px;">
                    <table style="table-layout:fixed; width:100%; border-collapse:collapse; border-spacing:0;">
                        <tbody>
                            ...
                            <tr>
                                <th style="font-size:14px; color:#222; font-weight:400; line-height:1.5; letter-spacing:-.02em; padding:4px 0; width:115px; text-align:left;">행사 일시</th>
                                <td style="font-size:14px; color:#222; font-weight:700; line-height:1.5; letter-spacing:-.02em; padding:4px 0;">{{ $eventDateText }}</td>
                            </tr>
                            <tr>
                                <th style="font-size:14px; color:#222; font-weight:400; line-height:1.5; letter-spacing:-.02em; padding:4px 0; width:115px; text-align:left;">행사 장소</th>
                                <td style="font-size:14px; color:#222; font-weight:700; line-height:1.5; letter-spacing:-.02em; padding:4px 0;">{{ $eventPlace }}</td>
                            </tr>
                            <tr>
                ...
                <div style="text-align:center; padding:54px 0 14px;">
                    <a href="{{ url('/mypage/participation_history') }}" target="_blank" style="font-size:14px; color:#fff; font-weight:600; line-height:1; padding:14px 45px; background:#0088B8; border-radius:8px; text-decoration:none;">등록 내역 확인하기</a>
                </div>
            </td>
        </tr>
        <tr>
