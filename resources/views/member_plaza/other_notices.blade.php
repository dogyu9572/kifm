...
<section class="scon" aria-labelledby="society-notices-heading">
	<div class="inner">
		...
		<div class="board_top">
			...
			<form method="GET" action="{{ route('member_plaza.other_notices') }}" class="right flex">
				...
				<div class="search_area">
					<label for="board-search" class="sound_only">기타 공지 검색</label>
				...
				<tbody>
					@forelse ($posts as $post)
						<tr class="{{ $post->is_notice ? 'notice' : '' }}">
							...
							@endif
							<td class="tal"><a href="{{ route('member_plaza.other_notices_show', $post->id) }}">{{ $post->title }}</a></td>
