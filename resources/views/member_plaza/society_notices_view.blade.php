	...
	<div class="inner">
		...
		<div class="board_view">
			<div class="tit_area">
				...
				<x-frontend.bookmark-button content-type="member_square_notices" :content-id="$post->id" :title="$post->title" :menu-label="$sName" :url="route('member_plaza.society_notices_show', $post->id)" />
			</div>
			...
			<div class="prev_next">
				@if ($prev)
					<a href="{{ route('member_plaza.society_notices_show', $prev->id) }}" class="prev"><strong>이전 글</strong><p>{{ $prev->title }}</p></a>
				@else
					<a href="#this" class="prev" aria-disabled="true"><strong>이전 글</strong><p>이전 글이 없습니다.</p></a>
				@endif
				@if ($next)
					<a href="{{ route('member_plaza.society_notices_show', $next->id) }}" class="next"><strong>다음 글</strong><p>{{ $next->title }}</p></a>
				@else
		...
		<div class="board_bottom">
			<a href="{{ route('member_plaza.society_notices') }}" class="btn btn_wkk btn_list">목록</a>
		</div>
