...
<section class="scon" aria-labelledby="society-notices-heading">
	<div class="inner">
		...
		<div class="board_top">
			...
			<form method="GET" action="{{ route('member_plaza.society_notices') }}" class="right flex">
				...
				<tbody>
					@forelse ($posts as $post)
						<tr class="{{ $post->is_notice ? 'notice' : '' }}">
							...
							@endif
							<td class="tal"><a href="{{ route('member_plaza.society_notices_show', $post->id) }}">{{ $post->title }}</a></td>
