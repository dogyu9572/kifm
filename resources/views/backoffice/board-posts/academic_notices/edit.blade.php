...
@section('content')
@php
    $returnUrl = request('return_url', route('backoffice.board-posts.index', $board->slug ?? 'academic_notices'));
@endphp
<div class="board-container">
    ...
    <div class="board-card">
        <div class="board-card-body">
            ...
            <form action="{{ route('backoffice.board-posts.update', [$board->slug ?? 'academic_notices', $post->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="board-form-group">
                    ...
                    <select class="board-form-control" id="event_id" name="event_id" required>
                        ...
                        @foreach ($events ?? [] as $ev)
                            <option value="{{ $ev->id }}" @selected((string) old('event_id', $post->event_id ?? '') === (string) $ev->id)>
                                {{ $ev->year }} {{ $ev->title }}
                            </option>
                        @endforeach
                    </select>
