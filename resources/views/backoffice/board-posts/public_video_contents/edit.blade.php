...
@section('content')
@php
    $returnUrl = request('return_url', route('backoffice.board-posts.index', $board->slug ?? 'public_video_contents'));
    $customFields = $post->custom_fields ? json_decode($post->custom_fields, true) : [];
    $youtubeUrl = $customFields['youtube_url'] ?? '';
@endphp
<div class="board-container">
    ...
    <div class="board-card">
        <div class="board-card-body">
            ...
            <form action="{{ route('backoffice.board-posts.update', [$board->slug ?? 'public_video_contents', $post->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="board-form-group">
                    ...
                    <input type="url" class="board-form-control" id="custom_field_youtube_url" name="custom_field_youtube_url" value="{{ old('custom_field_youtube_url', $youtubeUrl) }}" maxlength="500" placeholder="https://www.youtube.com/watch?v=...">
                </div>
