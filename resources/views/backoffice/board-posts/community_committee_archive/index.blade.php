...
@section('title', $board->name ?? '위원회 자료실')
    ...
    <div class="board-container">
        <div class="board-page-header">
            <div class="board-page-buttons">
                ...
                <a href="{{ route('backoffice.board-posts.create', $board->slug ?? 'community_committee_archive') }}" class="btn btn-success">
        ...
        <div class="board-card">
            <div class="board-card-body">
                <div class="board-filter">
                    <form method="GET" action="{{ route('backoffice.board-posts.index', $board->slug ?? 'community_committee_archive') }}" class="filter-form">
                        <div class="filter-row">
                            <div class="filter-group">
                                <label for="category" class="filter-label">위원회</label>
                                <select id="category" name="category" class="filter-select">
                                    <option value="">전체 위원회</option>
                                    @foreach ($board->getCategoryOptions() as $cat)
                                        <option value="{{ $cat->name }}" @selected(request('category') === $cat->name)>
                                            {{ $cat->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="filter-group">
                                <div class="filter-buttons">
                                    ...
                                    <a href="{{ route('backoffice.board-posts.index', $board->slug ?? 'community_committee_archive') }}" class="btn btn-secondary">
                ...
                <div class="board-list-header">
                    ...
                    <div class="list-controls">
                        <form method="GET" action="{{ route('backoffice.board-posts.index', $board->slug ?? 'community_committee_archive') }}" class="per-page-form">
                    ...
                    <table class="board-table">
                        <thead>
                            <tr>
                                ...
                                <th class="w12">작성자</th>
                                <th class="w12">작성일</th>
                                @if($board->enable_comments ?? false)
                                    <th class="w8">댓글</th>
                                @endif
                        ...
                        <tbody>
                            @forelse($posts as $post)
                                <tr>
                                    ...
                                    <td>{{ $post->category ?: '-' }}</td>
                                    <td>
                                        <span class="status-badge {{ $post->is_notice ? 'status-notice' : 'status-general' }}">
                                    ...
                                    <td>
                                        <div class="board-btn-group">
                                            <a href="{{ route('backoffice.board-posts.edit', [$board->slug ?? 'community_committee_archive', $post->id, 'return_url' => request()->fullUrl()]) }}" class="btn btn-primary btn-sm">
                                                <i class="fas fa-edit"></i> 수정
                                            </a>
                                            <form action="{{ route('backoffice.board-posts.destroy', [$board->slug ?? 'community_committee_archive', $post->id]) }}" method="POST" class="d-inline js-delete-confirm-form">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ ($board->enable_comments ?? false) ? 10 : 9 }}" class="text-center">등록된 자료가 없습니다.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
