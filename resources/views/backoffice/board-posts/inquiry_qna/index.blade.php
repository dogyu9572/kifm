...
@section('title', $board->name ?? 'Q&A 관리')
    ...
    <div class="board-container">
        <div class="board-page-header">
            <div class="board-page-buttons">
                ...
                <a href="{{ route('backoffice.board-posts.create', $board->slug ?? 'inquiry_qna') }}" class="btn btn-success">
        ...
        <div class="board-card">
            <div class="board-card-body">
                <div class="board-filter">
                    <form method="GET" action="{{ route('backoffice.board-posts.index', $board->slug ?? 'inquiry_qna') }}" class="filter-form">
                        <div class="filter-row">
                            ...
                            <div class="filter-group">
                                <label for="visibility" class="filter-label">공개 여부</label>
                            ...
                            <div class="filter-group">
                                ...
                                <select id="search_type" name="search_type" class="filter-select">
                                    <option value="keyword" @selected(request('search_type', 'keyword') === 'keyword')>전체</option>
                            ...
                            <div class="filter-group">
                                <div class="filter-buttons">
                                    ...
                                    <a href="{{ route('backoffice.board-posts.index', $board->slug ?? 'inquiry_qna') }}" class="btn btn-secondary">
                ...
                <div class="board-list-header">
                    ...
                    <div class="list-controls">
                        <form method="GET" action="{{ route('backoffice.board-posts.index', $board->slug ?? 'inquiry_qna') }}" class="per-page-form">
                        ...
                        <tbody>
                            @forelse($posts as $post)
                                <tr>
                                    ...
                                    <td><span class="status-badge {{ $post->is_notice ? 'status-notice' : 'status-general' }}">{{ $post->is_notice ? '공지' : '일반' }}</span></td>
                                    <td>{{ $post->title }}</td>
                                    ...
                                    <td>
                                        <div class="board-btn-group">
                                            <a href="{{ route('backoffice.board-posts.edit', [$board->slug ?? 'inquiry_qna', $post->id, 'return_url' => request()->fullUrl()]) }}" class="btn btn-primary btn-sm">
                                                <i class="fas fa-edit"></i> 수정
                                            </a>
                                            <form action="{{ route('backoffice.board-posts.destroy', [$board->slug ?? 'inquiry_qna', $post->id]) }}" method="POST" class="d-inline" onsubmit="return confirm('정말 이 게시글을 삭제하시겠습니까?');">
                                                @csrf
                                                @method('DELETE')
