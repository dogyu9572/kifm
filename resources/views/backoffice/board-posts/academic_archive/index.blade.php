    ...
    <div class="board-container">
        <div class="board-page-header">
            <div class="board-page-buttons">
                ...
                <a href="{{ route('backoffice.board-posts.create', $board->slug ?? 'academic_archive') }}" class="btn btn-success">
        ...
        <div class="board-card">
            <div class="board-card-body">
                <div class="board-filter">
                    <form method="GET" action="{{ route('backoffice.board-posts.index', $board->slug ?? 'academic_archive') }}" class="filter-form">
                        <div class="filter-row">
                            ...
                            <div class="filter-group">
                                <label for="member_grade" class="filter-label">회원설정</label>
                                <select id="member_grade" name="member_grade" class="filter-select">
                                    <option value="">전체</option>
                                    <option value="associate" @selected(request('member_grade') === 'associate')>준회원</option>
                                    <option value="regular" @selected(request('member_grade') === 'regular')>정회원</option>
                        ...
                        <div class="filter-row">
                            ...
                            <div class="filter-group">
                                <div class="filter-buttons">
                                    ...
                                    <a href="{{ route('backoffice.board-posts.index', $board->slug ?? 'academic_archive') }}" class="btn btn-secondary">
                ...
                <div class="board-list-header">
                    ...
                    <div class="list-controls">
                        <form method="GET" action="{{ route('backoffice.board-posts.index', $board->slug ?? 'academic_archive') }}" class="per-page-form">
                            ...
                            @forelse($posts as $post)
                                @php
                                    $customFields = is_string($post->custom_fields ?? null)
                                        ...
                                        : ($post->custom_fields ?? []);
                                    $memberTypeValue = $customFields['member_type'] ?? null;
                                    $memberType = is_array($memberTypeValue) ? implode(', ', $memberTypeValue) : ($memberTypeValue ?: '-');
                                    if (is_array($memberTypeValue)) {
                                        $memberType = collect($memberTypeValue)->map(function ($value) {
                                            return match($value) {
                                                'associate' => '준회원',
                                                'regular' => '정회원',
                                                'lifetime' => '평생회원',
                                        ...
                                        $memberType = match($memberTypeValue) {
                                            'associate' => '준회원',
                                            'regular' => '정회원',
                                            'lifetime' => '평생회원',
                                            'senior' => '시니어회원',
                                            null => '-',
                                            default => $memberTypeValue,
                                        };
                                    }
                                ...
                                <tr @if($board->enable_sorting) data-post-id="{{ $post->id }}" @endif>
                                    ...
                                    <td>
                                        <span class="status-badge status-general">{{ $post->is_notice ? '공지' : '일반' }}</span>
                                    </td>
                                    <td>{{ $memberType }}</td>
                                    ...
                                    <td>
                                        <div class="board-btn-group">
                                            <a href="{{ route('backoffice.board-posts.edit', [$board->slug ?? 'academic_archive', $post->id]) }}" class="btn btn-primary btn-sm">
                                                <i class="fas fa-edit"></i> 수정
                                            </a>
                                            <form action="{{ route('backoffice.board-posts.destroy', [$board->slug ?? 'academic_archive', $post->id]) }}" method="POST" class="d-inline" onsubmit="return confirm('정말 이 게시글을 삭제하시겠습니까?');">
                                                @csrf
                                                @method('DELETE')
