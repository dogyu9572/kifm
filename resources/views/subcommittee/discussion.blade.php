...
<section class="scon" aria-labelledby="subcommittee-heading">
	<div class="inner">
		...
		<div class="board_top">
			...
			<form method="GET" action="{{ route('subcommittee.discussion', $committee) }}" class="right flex">
		...
		<div class="board_list">
			<table>
				<caption>{{ $committee->name }} 토론장 목록입니다.</caption>
				<colgroup>
					<col class="num">
					<col>
					<col class="date">
				...
				<tbody>
					@forelse ($posts as $post)
						<tr class="{{ $post->is_notice ? 'notice' : '' }}">
							...
							@endif
							<td class="tal"><a href="{{ route('subcommittee.discussion_show', [$committee, $post->id]) }}">{{ $post->title }}</a></td>
