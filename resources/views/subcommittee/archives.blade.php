...
<section class="scon" aria-labelledby="subcommittee-heading">
	<div class="inner">
		...
		<div class="board_top">
			...
			<form method="GET" action="{{ route('subcommittee.archives', $committee) }}" class="right flex">
				...
				<div class="search_area">
					<label for="subcommittee-archives-search" class="sound_only">자료실 검색</label>
					<input type="text" id="subcommittee-archives-search" name="keyword" class="text" value="{{ request('keyword') }}" placeholder="검색어를 입력해 주세요">
		...
		<div class="board_list">
			<table>
				<caption>{{ $committee->name }} 자료실 목록입니다.</caption>
				<colgroup>
					<col class="num">
					<col>
					<col class="date">
				...
				<tbody>
					@forelse ($posts as $post)
						<tr class="{{ $post->is_notice ? 'notice' : '' }}">
							...
							@endif
							<td class="tal"><a href="{{ route('subcommittee.archives_show', [$committee, $post->id]) }}">{{ $post->title }}</a></td>
